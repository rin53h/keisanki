...
import src.my_library.count_statistics as counter 
...
class DataPreprocessor:
    def __init__(self, sentence_arrays, dictionary):
        ...
        """
        print(">>> doing:\t initiate DataPreprocessor")
        ...
        print(">>> done:\t initiate DataPreprocessor")

    def preprocess_data(self):
        """
        data.txtのデータ形式を前提に前処理を行う
        """
        print(">>> doing: preprocess data")
    ...
    def preprocess_data_and_label(self):
        ...
        """
        print("doing: preprocess data and lable")
        X_list, y_list = [], []
        for i in range(len(self.sentence_arrays)):
            ...
            if self.sentence_arrays[i][4] == "0":  # Writer_Joyの列が"0"なら負例とする 
                ...
                y_list.append(1)
            if len(X_list) % 100 == 0:
                print(f"preprocessing {len(X_list)}/{len(self.sentence_arrays)} : {X_list[-1]}, {y_list[-1]}")
        self.X, self.y = np.array(X_list), np.array(y_list)  # リストをnumpyの多次元配列に変換
        print("done: preprocess data and lable")
    ...
    def dump(self, file_path):
        ...
        """
        print("doing:\t save preprocessed data")
        with open(file_path, "wb") as f:
            pickle.dump((self.X, self.y), f)
        print("done:\t save preprocessed data")